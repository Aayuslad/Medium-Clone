...
const SearchBox = () => {
	...
	const formik = useFormik({
		...
		onSubmit: async (values) => {
			if (values.searchQuery === "") return;

			setResult(undefined);
			setResultBox(false);

			navigate(`/SearchResult/${values.searchQuery}/Stories`);
			formik.setValues({ searchQuery: "" });
		},
	});
	...
	return (
		<form className="searchBox items-center gap-2 relative w-full" onSubmit={formik.handleSubmit} ref={searchBoxRef}>
			...
			{resultBox && (result?.authors.length !== 0 || result?.stories.length !== 0 || result?.topics.length !== 0) && (
